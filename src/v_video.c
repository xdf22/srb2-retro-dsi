...
static void LoadPalette(const char *lumpname)
{
	...
	float mult = ((float)(cv_usegamma.value+8))*0.125f;
	if (mult < 1) mult = 1;
	...
	for (i = 0; i < palsize; i++)
	{
		pLocalPalette[i].s.red = (UINT8)min(255, (*pal) * mult);
		*pal++;
		pLocalPalette[i].s.green = (UINT8)min(255, (*pal) * mult);
		*pal++;
		pLocalPalette[i].s.blue = (UINT8)min(255, (*pal) * mult);
		*pal++;
...
void VID_BlitLinearScreen_ASM(const UINT8 *srcptr, UINT8 *destptr, UINT16 width, UINT16 height, size_t srcrowbytes,
	size_t destrowbytes);
#define HAVE_VIDCOPY
#endif
